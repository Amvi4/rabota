<template>
    <div class="prevention-page">
        <div class="hero-section">
            <div class="hero-content">
                <h1>ПРОФИЛАКТИКА КАРИЕСА</h1>
                <RouterLink to="/appointment"><button class="main-btn">ЗАПИСАТЬСЯ НА ПРИЕМ</button></RouterLink>
            </div>
        </div>

        <div class="container">
            <div class="cont-text">
                <h2>ПРОФИЛАКТИЧЕСКИЕ МЕРОПРИЯТИЯ</h2>
                <div class="cont-btn">
                    <p>Регулярная профилактика - залог здоровых зубов. Мы предлагаем комплексные программы профилактики кариеса.</p>
                    <ul>
                        <li>Профессиональная гигиена полости рта</li>
                        <li>Фторирование зубов</li>
                        <li>Герметизация фиссур</li>
                        <li>Обучение правильной гигиене</li>
                        <li>Регулярные профилактические осмотры</li>
                    </ul>
                </div>
            </div>
        </div>

        <div class="container">
            <div class="cont-text">
                <h2>РЕКОМЕНДАЦИИ</h2>
                <div class="cont-btn">
                    <p>Следуйте нашим рекомендациям для поддержания здоровья зубов:</p>
                    <ul>
                        <li>Чистите зубы дважды в день</li>
                        <li>Используйте зубную нить ежедневно</li>
                        <li>Посещайте стоматолога каждые 6 месяцев</li>
                        <li>Соблюдайте правильное питание</li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</template>

<style scoped>

.hero-section {
    height: 900px;
    background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('../images/3.png');
    background-size: cover;
    background-position: center;
    display: flex;
    align-items: center;
    justify-content: center;
    position: relative;
}

.hero-content {
    text-align: center;
    padding: 0 20px;
    max-width: 800px;
}

.hero-content h1 {
    margin-bottom: 30px;
}

.hero-content p {
    font-size: 24px;
    line-height: 1.6;
}

.container {
    display: flex;
    flex-direction: column;
    gap: 100px;
    width: 100%;
    padding: 126px 0;
}

.cont-text {
    padding: 0 157px;
    display: flex;
    align-items: start;
    justify-content: space-between;
}

.cont-text h2 {
    width: 300px;
}

.cont-text p {
    width: 679px;
}

.cont-btn {
    display: flex;
    flex-direction: column;
    gap: 24px;
}

.cont-btn ul {
    display: flex;
    flex-direction: column;
    gap: 16px;
    width: 679px;
}

.cont-btn li {
    padding-left: 20px;
    position: relative;
}

.cont-btn li::before {
    content: "•";
    position: absolute;
    left: 0;
}
</style>